...
func localAccountTimezone() string {
	...
	defer key.Close()
	windowsZone, _, err := key.GetStringValue("TimeZoneKeyName")
	if err != nil {
		return fallbackAccountTimezone
	}
	if timezone := windowsIANAZones[strings.TrimSpace(windowsZone)]; timezone != "" {
		return timezone
	}
	return fallbackAccountTimezone
}

var windowsIANAZones = map[string]string{
