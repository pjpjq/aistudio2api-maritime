...
func discoverPlatform(chromeRoot string) ([]Account, error) {
	...
	for _, profile := range profiles {
		...
		accounts = append(accounts, Account{
			...
			Importable: tokenErr == nil && platformImportable(encrypted, bindingKey),
		})
	}
	return accounts, nil
}
...
func readTokenService(chromeRoot string, profile string) (string, []byte, []byte, error) {
	...
	if err != nil {
		...
		hasBindingKey = false
		rows, err = database.Query("SELECT service, encrypted_token FROM token_service")
		if err != nil {
			return "", nil, nil, fmt.Errorf("读取 %s token_service: %w", profile, err)
		}
	}
	...
	var scanErr error
	if hasBindingKey {
		scanErr = rows.Scan(&service, &encryptedToken, &bindingKey)
	} else {
		scanErr = rows.Scan(&service, &encryptedToken)
	}
	if scanErr != nil {
		return "", nil, nil, fmt.Errorf("解析 %s token_service: %w", profile, scanErr)
	}
	if rows.Next() {
	...
	if len(encryptedToken) == 0 {
		return "", nil, nil, fmt.Errorf("%s 的 token_service 缺少 encrypted_token", profile)
	}
